...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        i = py_cpu_nms(boxes.detach().cpu().numpy(), scores.detach().cpu().numpy(), iou_thres)
...
def py_cpu_nms(boxes, scores, thresh):
    """
    nms
    :param boxes: ndarray [x1,y1,x2,y2]
    :param scores: ndarray [scores]
    :param thresh: int
    :return: list[index]
    """
    x1 = boxes[:, 0]
    ...
    while order.size > 0:
        ...
        order = order[index + 1] # not including 0
