...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    sampler = torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else torch.utils.data.RandomSampler(dataset, False)
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        self.mosaic2 = self.augment and not self.rect  # load 4 images at a time into a mosaic (only during training)
    ...
    def __getitem__(self, index):
        ...
        hyp = self.hyp
        mosaic = self.mosaic2 and random.random() < hyp['mosaic']
        if mosaic:
            # Load mosaic
            ...
            if random.random() < hyp['mixup']:
                ...
                r = np.random.uniform(0.4, 0.6)  # mixup ratio, alpha=beta=8.0
        ...
        if self.augment:
            ...
            # Augment colorspace
            augment_hsv(img, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'], contrast=hyp['contrast'])
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if random.random() < hyp['mixup']:
                #img2, labels2 = load_mosaic(self, random.randint(0, len(self.labels) - 1))
                img2, labels2 = load_mosaic9(self, random.randint(0, len(self.labels) - 1))
                r = np.random.uniform(8.0, 8.0)  # mixup ratio, alpha=beta=8.0
                img = (img * r + img2 * (1 - r)).astype(np.uint8)
                labels = np.concatenate((labels, labels2), 0)
...
def augment_hsv(img, hgain=0.5, sgain=0.5, vgain=0.5, contrast=0.1):
    r = np.random.uniform(-1, 1, 4) * [hgain, sgain, vgain, contrast] + 1  # random gains
    hue, sat, val = cv2.split(cv2.cvtColor(img, cv2.COLOR_BGR2HSV))
    dtype = img.dtype  # uint8

    x = np.arange(0, 256, dtype=np.int16)
    lut_hue = ((x * r[0]) % 180).astype(dtype)
    lut_sat = np.clip(x * r[1] * r[3], 0, 255).astype(dtype)
    lut_val = np.clip(x * r[2] * r[3], 0, 255).astype(dtype)
