...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Multi-scale
            if opt.multi_scale:
                sz = random.randrange(imgsz * 0.5, imgsz * 1.5) // gs * gs  # size
                if sz > 1880:
                    sz = 1920 // gs * gs
            ...
            if rank in [-1, 0]:
                ...
                # Plot
                if plots and log_images > 0:
                    log_images = log_images - 1
                    if not os.path.exists(save_dir / f'epoch_{epoch}'):
                        os.mkdir(save_dir / f'epoch_{epoch}')
                    f = save_dir / f'epoch_{epoch}/train_batch{ni}.jpg'  # filename
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img-size', nargs='+', type=int, default=[1280, 1280*2 + 640], help='[train, test] image sizes')
    parser.add_argument('--rect', type=bool, default=False,  help='rectangular training')
    parser.add_argument('--resume', nargs='?', const=True, default=False, help='resume most recent training')
    parser.add_argument('--nosave', type=bool, default=False,  help='only save final checkpoint')
    parser.add_argument('--notest', type=bool, default=False,  help='only test final epoch')
    parser.add_argument('--noautoanchor', type=bool, default=False, help='disable autoanchor check')
    ...
    parser.add_argument('--image-weights', default=False, help='use weighted image selection for training')
    parser.add_argument('--device', default='', help='cuda device, i.e. 0 or 0,1,2,3 or cpu')
    parser.add_argument('--multi-scale', type=bool, default=False,  help='vary img-size +/- 50%%')
    ...
    parser.add_argument('--exist-ok', type=bool, default=False,  help='existing project/name ok, do not increment')
